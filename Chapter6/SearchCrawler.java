...
public class SearchCrawler extends JFrame
{
  ...
  private void actionSearch() {
    ...
    if (errorList.size() > 0) {
      ...
      // 발생한 에러들을 하나의 메시지로 합친다(concatenation)
      ...
      try {
        ...
        while ((line = reader.readLine()) != null) {
          if (line.indexOf("Disallow:") == 0) {
            ...
            // 공백 문자(whitespace characters) 제거
        ...
        /* 로봇 파일이 없을 떄 예외가 발생하므로 접근이 허락된 것으로 가정한다. */
        return true;
      }
    }
  ...
  private ArrayList retrieveLinks(URL pageUrl, String pageContents, HashSet crawledList,boolean limitHost)
  {
    ...
    while (m.find()) {
      ...
      // 절대 경로 및 상대 경로를 완전한(fully qualified) 링크로 바꾼다.
      if (link.indexOf("://") == -1) {
        // URL 절대 경로 처리
        if (link.charAt(0) == '/') {

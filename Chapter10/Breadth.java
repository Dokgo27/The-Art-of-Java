...
class FlightInfo1 {
  ...
  FlightInfo1(String f, String t, int d) {
    from = f;
    to = t;
    distance = d;
    skip = false;
  ...
  void isflight(String from, String to)
  {
    ...
    while((f = find(from)) != null) {
      ...
      if((dist = match(f.to, to)) != 0) {
        resetStck.push(f.to);
        btStack.push(new FlightInfo3(from, f.to, f.distance));
        btStack.push(new FlightInfo3(f.to, to, dist));
        return;
      }
    }
    ...
    for(; i!=0; i--)
      resetSkip((FlightInfo3) resetStck.pop());
